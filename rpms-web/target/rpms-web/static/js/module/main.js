//监听选中页签添加样式
layui.config({
    base: '../static/js/module/'   //navbar组件js所在目录
}).use('navbar', function() {
    var navbar = layui.navbar();
    navbar.set({
        elem: '#nav',
        url: "../util/menu"   //数据源地址，我用了本地写的json数据
    });
    navbar.render();
    //下面的部分不是必须的
        ...
        layui.use('element', function() {
            ...
            var active = {
                ...
                tabChange: function(id) {
                    //切换到指定Tab项
                    element.tabChange('demo2', id); //根据传入的id传入到指定的tab项
                },
                tabDelete: function (id) {
                    element.tabDelete("demo2", id);//删除
                }
                , tabDeleteAll: function (ids) {//删除所有
                    $.each(ids, function (i,item) {
                        element.tabDelete("demo2", item); //ids是一个数组，里面存放了多个id，调用tabDelete方法分别删除
                    })
                }
            };
