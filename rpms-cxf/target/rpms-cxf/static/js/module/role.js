layui.use(['table','jquery','layer','form'], function() {
    ...
    table.render({
        ...
        ,cols: [[
            ...
            ,{field:'name', width:80, title: '角色名称'}
            ,{field:'sn', width:80, title: 'sn'}
            ,{field:'permission', width:80, title: 'permission'}
            ,{field:'permissions', width:500, title: '权限',templet: function(d){
                    // console.debug(d.opt.optname);
                  var arr=  d.permissions;
                  var result='';
                  for(var i of arr){
                        result+=i.name+" ";
                  }
    ...
    var Allmethods={
        add(){
            ...
            layer.open({
                ...
                title:'添加支付方式',
                area: ['450px', '400px'],
                content: $("#roleFormDiv"), //这里content是一个普通的String
        ...
        delete(){
            ...
            layer.confirm('你确认删除吗?', {icon: 3, title:'提示'}, function(index){
                ...
                $.get("/role/delete",{"ids":ids.toString()},function(result){
                    if(result.success){
        ...
        update(){
            ...
            layer.open({
                ...
                offset: '100px',
                area: ['450px', '400px'],
                content: $("#roleFormDiv"), //通过id拿到弹出的表单
                success: function(layero, index){
                    //成功打开窗体 消除遮蔽
    ...
    form.on('submit(demo1)', function(data) {
        ...
        var url = "/role/save";
        $.post(url, {"str":JSON.stringify(data.field),"permissionList":ids.toString()}, function (result) {
            if(result.success){
                //提示用户信息，添加数据成功
                layer.msg("操作数据成功");
                //关闭窗口
